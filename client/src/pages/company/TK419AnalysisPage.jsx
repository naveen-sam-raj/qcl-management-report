...
const TK419AnalysisPage = ({ plantId = 'sa' }) => {
  ...
  const handleReset = useCallback(() => {
    ...
    setDateError(false);
    setSaveSuccess(false);
    showToast('TK 419 analysis form cleared successfully.', 'info');
  }, [showToast]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await api.post('/api/tk-419-analysis', payload);

      setSaveSuccess(true);
      showToast(response.data?.message || 'TK 419 Analysis data saved successfully!', 'success');
    } catch (err) {
      const msg =
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-1.5 text-xs text-slate-400 font-medium leading-tight">
                ...
                <span className="text-blue-600 font-semibold">TK 419</span>
              </div>
        ...
        >
          ...
          <div className="text-xs">
            ...
            <span className="text-emerald-700">
              TK 419 Analysis for <strong>{formatDateDisplay(date)}</strong> has been recorded.
            </span>
          </div>
          <button
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-xs overflow-hidden max-w-2xl mx-auto lg:mx-0">
        ...
        <div className="px-5 py-3 border-t border-slate-200 bg-slate-50/60 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-slate-500">
          <div className="flex items-center gap-2">
            ...
            <span>
              Values for <strong>TK 419</strong> (FNH₃, CNH₃). Numeric values only. Leave blank if not sampled.
            </span>
          </div>
          <div className="font-mono text-[11px] text-slate-400">
            Total Slots: {visibleRows.length} (24-hour cycle)
          </div>
        </div>
      </div>
    </div>
...
export default TK419AnalysisPage;
